...
def eval_one_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, result_dir=None):
    ...
    for i in range(iter_step):
        ...
        for j in pred_dict_src.keys():
            ...
            np.savez(open(os.path.join(str(data_dir), img_name + '.ppm.usp'), 'wb'), src_score=s1_src, src_point=p1_src, src_des=d1_src,
                     dst_score=s1_dst, dst_point=p1_dst, dst_des=d1_dst, mat=mat, img_wh=(w, h))
