...
def main():
    ...
    data_root = '../Data/SLAM/freiburg/'
    data_list = []
    with open(data_root + 'gray_list.txt', 'r') as f:
    ...
    with torch.no_grad():
        ...
        for img_name in data_list:
            img_path = data_root + img_name.rstrip()
            ...
            for j in pred_dict.keys():
                ...
                # loc = np.where(s1 > 0.7)
                #
                # # s_nms = utils.key_nms(np.reshape(s1, (new_h // 8, new_w // 8)))
                # # s_nms = np.reshape(s_nms, s1.shape)
                # # loc = np.where(s_nms > 0.7)
                #
                ...
                if vis_match and cur_index % vis_match_skip == 0:
                    if last_img is None:
                        last_s = s1
                        last_p = p1
                        last_d = d1
                ...
                for i in range(p1.shape[0]):
                    pos = p1[i]
                    cv2.circle(img, (int(pos[0]), int(pos[1])), 1, (0, 255, 0), -1)
                    # if i < 20:
