...
def compute_matching_score(data, keep_k_points=1000):
    ...
    true_warped_keypoints = warp_keypoints(m_warped_keypoints[:, :2], np.linalg.inv(real_H))
    vis_warped = np.all((true_warped_keypoints >= 0) & (true_warped_keypoints <= (np.array(shape)[::-1]-1)), axis=-1)
    ...
    true_keypoints = warp_keypoints(m_keypoints[:, :2], real_H)
    vis = np.all((true_keypoints >= 0) & (true_keypoints <= (np.array(shape)[::-1]-1)), axis=-1)
...
def compute_homography(data, keep_k_points=1000):
    ...
    H, inliers = cv2.findHomography(m_keypoints[:, :2], m_warped_keypoints[:, :2], cv2.RANSAC)
